...
router.get("/players/:season_id/pug", async (req, res) => {
  try {
    let seasonId = req.params.season_id;
    let leaderboard = await getPlayerLeaderboard(seasonId, true);
    res.json({ leaderboard });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: err });
...
const getPlayerLeaderboard = async (seasonId = null, pug = false) => {
   ...
   */
  let matchStatSql =
    `SELECT id FROM \`match\` WHERE cancelled=0 AND is_pug=` + pug;
  if (seasonId) matchStatSql += " AND season_id =" + seasonId;
  ...
  const matchStats = await db.query(matchStatSql);
  if (!matchStats.length) return allPlayers;
  playerStats = await db.query(playerStatSql, [matchStats.map(({ id }) => id)]);
    ...
    if (!allPlayers.some((el) => el.steamId === player.steam_id)) {
      allPlayers.push({
        ...
        average_rating: Utils.getElo(
          ...
          parseFloat(player.assists),
          parseFloat(player.hsk),
          parseFloat(player.k2),
          parseFloat(player.k3),
          parseFloat(player.k4),
          parseFloat(player.k5),
          parseFloat(player.v1),
          parseFloat(player.v2),
          parseFloat(player.v3),
          parseFloat(player.v4),
          parseFloat(player.v5),
          parseFloat(player.totalMaps),
          parseFloat(player.wins)
        ),
        wins: parseFloat(player.wins),
      ...
      collisionPlayer.wins += parseFloat(player.wins);
      ...
      collisionPlayer.average_rating = Utils.getElo(
        parseFloat(collisionPlayer.kills),
        parseFloat(collisionPlayer.deaths),
        parseFloat(collisionPlayer.assists),
        parseFloat(collisionPlayer.hsk),
        parseFloat(collisionPlayer.k2),
        parseFloat(collisionPlayer.k3),
        parseFloat(collisionPlayer.k4),
        parseFloat(collisionPlayer.k5),
        parseFloat(collisionPlayer.v1),
        parseFloat(collisionPlayer.v2),
        parseFloat(collisionPlayer.v3),
        parseFloat(collisionPlayer.v4),
        parseFloat(collisionPlayer.v5),
        parseFloat(collisionPlayer.total_maps),
        parseFloat(collisionPlayer.wins)
      );
